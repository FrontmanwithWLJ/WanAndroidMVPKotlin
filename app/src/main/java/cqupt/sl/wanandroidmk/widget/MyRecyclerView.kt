...
class MyRecyclerView : RecyclerView {
    ...
    override fun onLayout(changed: Boolean, l: Int, t: Int, r: Int, b: Int) {
        ...
        Log.e(
            "SL",
            "onLayout originRect.left = ${originRect.left},top=${originRect.top},right=${originRect.right},bottom=${originRect.bottom}"
        )
    }
    ...
    override fun dispatchTouchEvent(ev: MotionEvent?): Boolean {
        when (ev!!.action) {
            MotionEvent.ACTION_MOVE -> {
                //滑动到顶部可以下拉
                if (downEvent.y < ev.y && isCanPullDown()) {
                    if (!isParent) {
                        downEvent = ev.apply { action = MotionEvent.ACTION_DOWN }
                        isParent = true
                    }
                    Thread {
                ...
                //上拉到底部时
                else if (downEvent.y > ev.y && isCanPullUp()) {
                    if (!isParent) {
                        downEvent = ev.apply { action = MotionEvent.ACTION_DOWN }
                        isParent = true
                    }
...
//                Log.e(
//                    "SL",
//                    "this.left = ${this.left},top=${this.top},right=${this.right},bottom=${this.bottom}"
//                )
//                this.layout(originRect.left, originRect.top, originRect.right, originRect.bottom)
//                Log.e(
//                    "SL",
//                    "originRect.left = ${originRect.left},top=${originRect.top},right=${originRect.right},bottom=${originRect.bottom}"
//                )
//                canPullDown = false
//                canPullUp = false
//                isMoved = false
